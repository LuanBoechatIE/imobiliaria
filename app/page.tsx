...
function Delta({ pessoa }: { pessoa: Corretor }) {
  if (!pessoa.anterior) {
    return <span className={s.delta}>1º ciclo</span>;
  }
  const diff = media(pessoa.notas) - media(pessoa.anterior);
  if (Math.abs(diff) < 0.05) {
    return <span className={s.delta}>estável</span>;
  }
  const sobe = diff > 0;
  return (
    ...
    <main className={s.shell}>
      <header className={s.topo}>
        <div className={s.identidade}>
          ...
          <span className={s.local}>{cidade} · {corretores.length} corretores avaliados</span>
        </div>
      ...
      <section className={s.bloco}>
        ...
        <p className={s.blocoSub}>
          Média de cada competência, da mais fraca para a mais forte. A ordem aqui define
          o que entra no treino do mês.
        </p>
      ...
      <footer className={s.rodape}>
        ...
        <span>
          Imobiliária fictícia, números ilustrativos. Nenhum dado de cliente real.
        </span>
      </footer>
    </main>
  );
